...
NAN_METHOD(Open) {
  ...
  baton->callback.Reset(info[2].As<v8::Function>());
    ...
    baton->rs485_delay_after_send = getBoolFromObject(options, "rs485DelayAfterSend");
    baton->rs485_rx_during_send = getBoolFromObject(options, "rs485RxDuringSend");
    baton->rs485_enabled = getBoolFromObject(options, "rs485Enabled");
  #endif
